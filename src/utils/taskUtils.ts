...
export const sortTasksByPriority = (tasks: Task[]) => {
  return [...tasks].sort((a, b) => {
    ...
    // TIER 3: OVERDUE reapply callbacks jump here. Only reapply, only when
    // overdue (callback time already passed). Non-overdue reapply sorts normally.
    // The earlier leak (reapply surfacing to the wrong recruiter) is handled by
    // the defensive employee_id guard in filterActiveTasks.
    const aIsReapplyOverdue = a.apiData?.callbackType === 'reapply' && !!a.dueDate && a.dueDate.getTime() < nowMs;
    const bIsReapplyOverdue = b.apiData?.callbackType === 'reapply' && !!b.dueDate && b.dueDate.getTime() < nowMs;
    if (aIsReapplyOverdue && !bIsReapplyOverdue) return -1;
    if (!aIsReapplyOverdue && bIsReapplyOverdue) return 1;
    if (aIsReapplyOverdue && bIsReapplyOverdue) {
